...
Backend (/ingest)
  ...
  ├─→ notes/new/ に一時保存
  ├─→ 既存ID確認（増分更新）
  ├─→ 新出単語抽出（/ingest/analyze）
  │     ├─→ LLMで類似候補検索
  │     └─→ ユーザー判定（新規/表記揺れ/スキップ）
  ├─→ 正規化辞書更新（/dictionary/update）
  ├─→ Embedding生成 → ChromaDB追加
  ├─→ ChromaDB → GCS同期
  └─→ ファイル処理（processed/へ移動、削除しない）⭐ v3.0変更
  ↓
...
- `GET /dictionary/export` - 辞書エクスポート（YAML/JSON/CSV）
...
- **v3.0.2** (2025-12-30) - CLAUDE.md改善（作業単位ドキュメント管理、非IT系ユーザー向け配慮を明記）📝
- **v3.0.1** (2025-12-30) - ハイブリッド検索機能追加 ⭐
- **v3.0.0** (2025-12-29) - v1改善点実装完了（マルチテナント、ノート取り込み改善、モデル2段階選択など）
...
**作成日**: 2025-12-19
**最終更新**: 2025-12-30
**バージョン**: 3.0.2 (作業単位ドキュメント管理、非IT系ユーザー向け配慮を明記)
**管理者**: 開発チーム
